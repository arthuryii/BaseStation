...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        parms: [],                         //提交到服务器的参数
        columns: [],                          //数据源
        allowSelect: true,                       //Edited 是否允许选择行
    ...
    $.ligerDefaults.Grid_columns = {
        ...
        styleRender: null //Edited 列样式，输入列id，直接返回css即可
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadData: function (loadDataParm) {
            ...
            else if (typeof (loadDataParm) == "string" && loadDataParm == "search") {
                resetPage = true;
            }
            //参数初始化
        ...
        */
        ...
        _createHeaderCell: function (column) {
            ...
            if (column.styleRender) {//Edited styleRender
                var cssstr = column.styleRender();
                var cssarr = cssstr.split(';');
                for (var i in cssarr) {
                    if (cssarr[i].indexOf(':') != -1) {
                        var arr = cssarr[i].split(':');
                        if (arr.length == 2) {
                            jcell.css(arr[0], arr[1]);
                        }
                    }
                }
            }
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                $(g.columns).each(function (columnindex, column) {
                    ...
                    if (column.styleRender) {//Edited styleRender
                        gridhtmlarr.push(column.styleRender());
                    }
                    gridhtmlarr.push(' ">');
        ...
        _onClick: function (e) {
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                if (src.checkbox || !selectRowButtonOnly) {
                    ...
                    var isTrue = true;
                    g.gridview1.find(".l-grid-row").each(function () { isTrue = isTrue && $(this).hasClass("l-selected"); })
                    if (isTrue)
                        g.gridview1.find(".l-grid-hd-row").addClass("l-checked");
                    else
                        g.gridview1.find(".l-grid-hd-row").removeClass("l-checked");
                }
        ...
        select: function (rowParm) {
            if (this.options.allowSelect) {//Edited allowSelect匹配
                var g = this, p = this.options;
                var rowdata = g.getRow(rowParm);
                var rowid = rowdata['__id'];
                var rowobj = g.getRowObj(rowid);
