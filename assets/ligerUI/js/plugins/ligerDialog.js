...
(function ($)
{
    ...
    $.ligerDialog.wait = function (title, timeout)//Edited 增加timeout参数，按毫秒表示
    {
        if (!timeout)
            timeout = 1000;
        title = title || $.ligerDefaults.Dialog.waittingMessage;
        var dialog = $.ligerDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
        setTimeout(function () { dialog.close(); }, timeout);
        return dialog;
    };
