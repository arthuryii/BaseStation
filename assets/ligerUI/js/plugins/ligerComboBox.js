...
(function ($) {
    ...
    $.ligerDefaults.ComboBox = {
        ...
        orijson: null,
        searchHeaders: [] //Edited searchHeaders
    };
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            ...
            if (this.element.tagName.toLowerCase() == "input") {
                this.element.readOnly = !p.autocomplete;
            ...
            g.inputText.keyup(function () {
                setTimeout(function () {
                    ...
                    if (p.url) {
                        ...
                        g.set('url', p.url);
                        g.selectBox.show();
                    } else if (p.grid) {//Edited 修改了自动完成功能
                        if (p.searchHeaders.length > 0) {
                            g.grid.options.data = eval('(' + p.orijson + ')');
                            g.grid.reload(function (data) {
                                var txt = g.getText().toUpperCase();
                                for (var i = 0; i < p.searchHeaders.length; i++) {
                                    var d = data[p.searchHeaders[i]];
                                    if (d) {
                                        if (d.toUpperCase().indexOf(txt) != -1)
                                            return true;
                                    }
                                }
                                return false;
                        ...
                        var txt = g.getText();
                        var data = $.grep(eval("(" + p.orijson + ")"), function (data) {
                            return data.CompanyName.indexOf(txt) != -1;
                        });
                        if (data.length == 0)
                            g.data = [];
                        else
